...
class Homefilmes(LoginRequiredMixin, ListView):
    ...
    model = Filme
    # object_list -> lista de obejtos do modelo


class Detalhesfilme(LoginRequiredMixin, DetailView):
    template_name = "detalhesfilme.html"
    model = Filme
    # object -> um objeto do modelo


    def get(self, request, *args, **kwargs):
        #contabilizando uma visualizacao
        filme = self.get_object()
        filme.visualizacoes += 1
        filme.save()
        usuario = request.user
        usuario.filmes_vistos.add(filme)
        return super().get(request, *args, **kwargs) # redirecionando o usuario para a url final
...
class DeletarConta(DeleteView):
    model = Usuario
    template_name = 'excluirconta.html'
